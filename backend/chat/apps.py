...
def is_serving() -> bool:
    ...
    point spending 270 MB on them. Under runserver's autoreloader only the
...
def warm_retrieval() -> None:
    ...
    langchain is itself a second or two; running the whole thing in a thread
    keeps that off both the startup path and the first request.
    """
    try:
...
class ChatConfig(AppConfig):
    ...
    def ready(self):
        ...
        # something uncached pays for it. Cached answers never wait either way,
        # being served before any of this is touched.
        if is_serving():
